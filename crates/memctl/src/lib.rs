    ...
    async fn repair_vfs_tree(client: &LocalClient) -> Result<usize> {
        ...
        for &dup_bytes in &root_candidates[1..] {
            ...
            for (src, edge) in &edges {
                ...
                if client
                    .add_link(&root_ref, new_edge, Visibility::Internal)
                    .await
                    .is_ok()
                {
            ...
            while let Some(current) = dedup_stack.pop() {
                ...
                for (name, mut entries) in by_name {
                    ...
                    for (dup_eid, _) in &entries[1..] {
                        ...
                        {
                            println!(
                                "  Retracted duplicate edge for \"{name}\" (edge {})",
                                hex::encode(dup_eid)
                            );
                            actions += 1;
                        }
        ...
        let retracted: std::collections::HashSet<[u8; 32]> =
            root_candidates[1..].iter().copied().collect();
        for (id, name) in &all_dirs {
            ...
            if client
                .add_link(&root_ref, edge, Visibility::Internal)
                .await
                .is_ok()
            {
