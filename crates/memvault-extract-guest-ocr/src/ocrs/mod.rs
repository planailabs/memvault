//! Vendored copy of the `ocrs` OCR engine.
//!
//! Source: <https://github.com/robertknight/ocrs>, `ocrs/src` at rev
//! `459cced005dbf5058954fcac85106f49fc0835ad` (post-0.12.2 main).
//! License: MIT OR Apache-2.0, Copyright (c) Robert Knight.
//!
//! Why vendored instead of a crates.io dependency: ocrs unconditionally
//! links `wasm-bindgen` on every wasm32 target (its browser `wasm_api`).
//! wasm-bindgen's runtime objects leave `__wbindgen_placeholder__.*` /
//! `__wbindgen_externref_xform__.*` imports in the final module that only
//! the wasm-bindgen CLI post-processor can resolve — the Extism host
//! rejects the module at instantiation. This copy drops `wasm_api`, the
//! `log` module's JS console binding, and the wasm-bindgen dependency;
//! `crate::` paths were rewritten to `crate::ocrs::`. Everything else is
//! unmodified. Switch back to crates.io once a release ships with the
//! `export-wasm` feature made optional for non-browser wasm targets.
