...
fn person(name: &str) -> Entity {
    ...
    Entity { id: EntityId::random(), kind: "person".to_string(), props, edges_out: vec![] }
}
...
async fn add_link_between_entities() {
    ...
    let eid = node.client.add_link(&NodeRef::Entity(a.clone()), edge, Visibility::Internal).await.unwrap();
...
async fn list_edges_of_entity() {
    ...
    for target in [&b, &c] {
        ...
        node.client.add_link(&NodeRef::Entity(a.clone()), edge, Visibility::Internal).await.unwrap();
    }
...
async fn remove_link() {
    ...
    node.client.add_link(&NodeRef::Entity(a.clone()), edge, Visibility::Internal).await.unwrap();
    node.client.remove_link_from(&NodeRef::Entity(a.clone()), &edge_id).await.unwrap();
...
async fn traverse_graph() {
    ...
    let hits = node.client.traverse_from(&NodeRef::Entity(a), Some("knows"), 2).await.unwrap();
