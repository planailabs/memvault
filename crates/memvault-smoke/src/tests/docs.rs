...
async fn create_and_get_doc() {
    ...
    node.client.put_doc(doc.clone(), vec![], Visibility::Internal).await.unwrap();
...
async fn create_doc_with_tags() {
    ...
    node.client.put_doc(doc.clone(), tags, Visibility::Internal).await.unwrap();
    let docs = node.client.list_docs(Some(("topic".into(), "rust".into())), 100).await.unwrap();
...
async fn list_docs_by_tag() {
    ...
    for i in 0..5 {
        ...
        node.client.put_doc(doc, vec![("batch".into(), "alpha".into())], Visibility::Internal).await.unwrap();
    }
    for i in 0..3 {
        let doc = Document::new(DocId::random(), format!("other-{i}"), Default::default());
        node.client.put_doc(doc, vec![("batch".into(), "beta".into())], Visibility::Internal).await.unwrap();
    }
    let alpha = node.client.list_docs(Some(("batch".into(), "alpha".into())), 100).await.unwrap();
    let beta = node.client.list_docs(Some(("batch".into(), "beta".into())), 100).await.unwrap();
...
async fn list_all_docs() {
    ...
    let all = node.client.list_docs(None, 100).await.unwrap();
    assert_eq!(all.len(), 7);
}
...
async fn doc_visibility_federated() {
    ...
    node.client.put_doc(doc.clone(), vec![], Visibility::Federated).await.unwrap();
...
async fn doc_visibility_public() {
    ...
    node.client.put_doc(doc.clone(), vec![], Visibility::Public).await.unwrap();
    assert!(node.client.get_doc(&doc.id).await.unwrap().is_some());
}
...
async fn two_nodes_docs_isolated() {
    ...
    node_a.client.put_doc(doc_a.clone(), vec![], Visibility::Internal).await.unwrap();
    node_b.client.put_doc(doc_b.clone(), vec![], Visibility::Internal).await.unwrap();
...
async fn create_100_docs() {
    ...
    for i in 0..100 {
        ...
        node.client.put_doc(doc, vec![("bulk".into(), "yes".into())], Visibility::Internal).await.unwrap();
    }
    let docs = node.client.list_docs(Some(("bulk".into(), "yes".into())), 200).await.unwrap();
