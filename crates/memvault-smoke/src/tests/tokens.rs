...
async fn issue_token() {
    ...
    let token = node.client.issue_token(Role::AgentHost, 3600, 1, Some("test".into())).await.unwrap();
...
async fn list_tokens() {
    let node = TestNode::new();
    node.client.issue_token(Role::AgentHost, 3600, 1, Some("first".into())).await.unwrap();
    node.client.issue_token(Role::AgentHost, 7200, 5, Some("second".into())).await.unwrap();
...
async fn revoke_token() {
    let node = TestNode::new();
    node.client.issue_token(Role::AgentHost, 3600, 1, Some("revocable".into())).await.unwrap();
    let tokens = node.client.list_tokens().await.unwrap();
    let cid = tokens[0].cid.clone();
    node.client.revoke_token(&cid, "no longer needed").await.unwrap();
    let tokens = node.client.list_tokens().await.unwrap();
...
async fn token_with_max_uses() {
    let node = TestNode::new();
    node.client.issue_token(Role::AgentHost, 3600, 10, Some("multi".into())).await.unwrap();
    let tokens = node.client.list_tokens().await.unwrap();
...
async fn token_decode_roundtrip() {
    let node = TestNode::new();
    let encoded = node.client.issue_token(Role::AgentHost, 3600, 1, Some("roundtrip".into())).await.unwrap();
