...
pub trait MemvaultClient: Send + Sync {
    ...
    // ── High-level helpers (default impls delegate to free fns) ────────
    //
    // These compose lower-level trait methods. Default impls are provided
    // so any MemvaultClient gets them for free. Concrete impls may override
    // for transport-specific optimisations (e.g. a server-side fast path).
    ...
    async fn create_doc(
        ...
        body: &str,
        title: Option<&str>,
        frontmatter: Option<std::collections::BTreeMap<String, serde_json::Value>>,
        tags: Vec<(String, String)>,
        vis: Visibility,
        vfs_path: Option<&str>,
        bucket: Option<&BucketId>,
    ) -> Result<crate::docs::CreateDocResult> {
        crate::docs::create_doc(self, body, title, frontmatter, tags, vis, vfs_path, bucket).await
    }

    /// Upload a file, optionally linking it into the VFS.
    ...
    async fn upload_file_at(
        ...
        crate::files::upload_file(self, data, filename, mime_type, tags, visibility, vfs_path, bucket).await
    }

    // -- VFS helpers --

    async fn vfs_default_bucket(&self) -> BucketId {
        crate::vfs::default_bucket(self).await
    }
    async fn vfs_ensure_root(&self, bucket: &BucketId) -> Result<memvault_core::EntityId> {
        crate::vfs::ensure_root(self, bucket).await
    }
    async fn vfs_resolve(
        &self,
        bucket: &BucketId,
        path: &str,
    ) -> Result<Option<(memvault_core::NodeRef, Option<memvault_core::EdgeId>)>> {
        crate::vfs::resolve_path(self, bucket, path).await
    }
    async fn vfs_list_children(
        &self,
        parent: &memvault_core::NodeRef,
    ) -> Result<Vec<(String, memvault_core::NodeRef, memvault_core::EdgeId)>> {
        crate::vfs::list_children(self, parent).await
    }
    async fn vfs_find_child(
        &self,
        parent: &memvault_core::NodeRef,
        name: &str,
    ) -> Result<Option<(memvault_core::NodeRef, memvault_core::EdgeId)>> {
        crate::vfs::find_named_child(self, parent, name).await
    }
    async fn vfs_ensure_dir_path(
        &self,
        bucket: &BucketId,
        path: &str,
    ) -> Result<memvault_core::NodeRef> {
        crate::vfs::ensure_dir_path(self, bucket, path).await
    }
    async fn vfs_link_at_path(
        &self,
        bucket: &BucketId,
        path: &str,
        target: &memvault_core::NodeRef,
    ) -> Result<memvault_core::EdgeId> {
        crate::vfs::link_at_path(self, bucket, path, target).await
    }
    async fn vfs_link_node_at_path(
        &self,
        bucket: &BucketId,
        path: &str,
        node_id: &str,
    ) -> Result<()> {
        crate::vfs::link_node_at_path(self, bucket, path, node_id).await
    }
    async fn vfs_resolve_node_type(&self, node: &memvault_core::NodeRef) -> String {
        crate::vfs::resolve_node_type(self, node).await
    }
    async fn vfs_mkdir(
        &self,
        bucket: &BucketId,
        path: &str,
    ) -> Result<memvault_core::EntityId> {
        crate::vfs::mkdir(self, bucket, path).await
    }
    async fn vfs_unlink_path(&self, bucket: &BucketId, path: &str) -> Result<()> {
        crate::vfs::unlink_path(self, bucket, path).await
    }
    async fn vfs_mv_path(&self, bucket: &BucketId, from: &str, to: &str) -> Result<()> {
