//! memctl -- CLI for managing memvault.
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Genesis { admin_key } => {
            ...
            // Write the cluster_id file.
            let id_path = data_dir.join("cluster_id");
            std::fs::write(&id_path, id_hex.as_bytes())?;

            // Create the identity directory (keys generated later).
            let identity_dir = data_dir.join("identity");
            std::fs::create_dir_all(&identity_dir)?;

            // Create the trust directory (genesis attestation stored later).
        ...
        Commands::Put {
            ...
            let tags: Vec<(String, String)> = tag
                .iter()
                .filter_map(|t| {
                    let (scope, label) = t.split_once(':')?;
                    Some((scope.to_string(), label.to_string()))
                })
                .collect();
        ...
        Commands::GraphAdd { kind, prop } => {
            ...
            let id = client
                .add_entity(entity, Visibility::Internal)
                .await?;
        ...
        Commands::GraphQuery {
            ...
            let client = create_client(store);
            let hits = client
                .traverse(&entity_id, relation.as_deref(), max_depth)
                .await?;
            for hit in hits {
                println!(
                    "depth={} entity={}",
                    hit.depth,
                    hex::encode(hit.entity_id.0)
                );
            }
        }
        ...
        Commands::RepairIndex => {
            println!("Index repair: rebuilding from blockstore...");
            println!("  (not yet implemented)");
        }
        Commands::RenewAttestation { peer_id } => {
            println!("Attestation renewal for {peer_id}: not yet implemented in standalone mode");
        }
    }
